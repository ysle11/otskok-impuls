...
//   Print(TXT);
  ...
  int cBuffer[1110];
  int dwBytesRead[1]; 
  handle=FileOpen("responseW1.txt", FILE_BIN|FILE_READ);dwBytesRead[0]=FileSize(handle) ;
  if(handle>0)
    {
     FileReadArray(handle, cBuffer, 0, 1110);
     FileClose(handle);
      ...
      while( index0 < StringLen(text))
      {
         ...
         s1=StringSubstr(text,index0,(index-index0));index0=index+2;
           ...
           if(op[ix]=="buy"){op1=OP_BUYSTOP;otp1=priceopen[ix]+pnt*15+pnt*MarketInfo(val[ix],MODE_SPREAD);}else if(op[ix]=="sell"){op1=OP_SELLSTOP;otp1=priceopen[ix]-pnt*15-pnt*MarketInfo(val[ix],MODE_SPREAD);clr=Red;}
           ...
           if(!isopened(val[ix],op1,priceopen[ix],otp1))
            {OrderSend(val[ix],op1,lots,priceopen[ix],3,0,otp1,NULL,magic,TimeCurrent()+(22-TimeHour(TimeCurrent()))*3600+86400*(5-DayOfWeek()) );added="+";}
