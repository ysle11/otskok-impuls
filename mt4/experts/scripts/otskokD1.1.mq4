     ...
     FileClose(handle);
      ...
      while( index0 < StringLen(text))
      {
         ...
         s1=StringSubstr(text,index0,(index-index0));index0=index+2;
           ...
           if(op[ix]=="buy"){op1=OP_BUYSTOP;otp1=priceopen[ix]+pnt*10+pnt*MarketInfo(val[ix],MODE_SPREAD);}else if(op[ix]=="sell"){op1=OP_SELLSTOP;otp1=priceopen[ix]-pnt*10-pnt*MarketInfo(val[ix],MODE_SPREAD);clr=Red;}
           ...
           if(!isopened(val[ix],op1,priceopen[ix],otp1))
            {OrderSend(val[ix],op1,lots,priceopen[ix],3,0,0,NULL,0,TimeCurrent()+(22-TimeHour(TimeCurrent()))*3600);added="+";}
...
bool isopened(string tval,int top,double tpriceopen,double totp1){
    ...
    for(int o=OrdersTotal()-1;o>=0;o--){
      ...
      if(OrderSymbol()==tval)if(OrderType()==top)
      {
         if(OrderOpenPrice()!=tpriceopen)OrderModify(OrderTicket(),tpriceopen,0,0,0);
         opened=true;
      }
    }   
   return (opened);
